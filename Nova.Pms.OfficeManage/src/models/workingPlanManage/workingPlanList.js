  ...
  effects: {
    *getWorkingPlanList (
      ...
      const {
        data,
        headers,
      } = yield call(workingPlanManageService.getWorkingPlanlist, {
        page,
        filterStr,
        pageSize,
      })
    ...
    *reviewWorkingPlan ({ payload: ids }, { call, put, select }) {
      const { data } = yield call(
        workingPlanManageService.reviewWorkingPlan,
        ids
      )
      message.success(data.message, 3)
      yield put({ type: 'reload' })
    ...
    *cancellationAuditWorkingPlan ({ payload: ids }, { call, put, select }) {
      const { data } = yield call(
        workingPlanManageService.cancellationAuditWorkingPlan,
        ids
      )
      message.success(data.message, 3)
      yield put({ type: 'reload' })
    ...
    *changeWorkingPlanState ({ payload: { changeStateModel } }, { call, put }) {
      const {
        data,
      } = yield call(workingPlanManageService.changeWorkingPlanState, {
        changeStateModel,
      })
      message.success(data.message, 3)
      yield put({ type: 'reload' })
