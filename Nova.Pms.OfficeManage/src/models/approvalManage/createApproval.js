...
export default {
  ...
  reducers: {
    ...
    changeField (state, { payload: { key, value } }) {
      const approval = { ...state.approval, [key]: value }
    ...
    sendIdDeselect (state, { payload }) {
      let person = state.personStatus.find(
        person =>
          person.personId == payload.value && person.approvalPersonType == 2
      )
    ...
    approvalIdDeselect (state, { payload }) {
      let person = state.personStatus.find(
        person =>
          person.personId == payload.value && person.approvalPersonType == 1
      )
      var index = state.personStatus.indexOf(person)
      if (index >= 0) {
        state.personStatus.splice(index, 1)
    ...
    removeAttachment (state, { payload: model }) {
      let file = state.attachments.find(file => file.uid == model.uid)
  ...
  effects: {
    *getData ({ payload: { page = 1, filterStr = '' } }, { call, put }) {
      ...
      const code =
        padStr(date.getFullYear()).toString() +
        padStr(date.getMonth() + 1).toString() +
