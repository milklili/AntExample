import * as securityEquipMentManageService
  from '../../services/securityEquipMentManage'
...
export default {
  ...
  reducers: {
    ...
    regionChanged (state, { payload }) {
      ...
      let securityEquipMent = {
        ...state.securityEquipMent,
        ...initialSelected,
      }
    ...
    changeField (state, { payload: { key, value } }) {
      const securityEquipMent = { ...state.securityEquipMent, [key]: value }
  ...
  effects: {
    ...
    *addSecurityEquipMent ({ payload: values }, { call, put, select }) {
      ...
      const { data } = yield call(securityEquipMentManageService.create, {
        val,
      })
    ...
    *selectRegion ({ payload: id }, { put, call }) {
      const {
        data: departmentList,
      } = yield call(commonDataService.getDepartmentByRegionId, { id })
      const {
        data: staffList,
