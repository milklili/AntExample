...
export default {
  ...
  reducers: {
    updateState (
      state,
      { payload: { data: list, total, page, filterStr, staffList, pageSize } }
    ) {
  ...
  effects: {
    *getData (
      ...
      const { data } = yield call(workAttendanceService.getData, {
        page,
        filterStr,
        pageSize,
      })
    ...
    *addWorkAttendance ({ payload: addWorkAttendance }, { call, put, select }) {
      const { data } = yield call(
        workAttendanceService.addWorkAttendance,
        addWorkAttendance
      )
      message.success(data.message, 3)
      yield put({ type: 'reload' })
    ...
    *deleteDocumentAttachments ({ payload: ids }, { call, put, select }) {
      const { data } = yield call(
        documentService.deleteDocumentAttachments,
        ids
      )
      message.success(data.message, 3)
      yield put({ type: 'reload' })
    ...
    *addFiles ({ payload: values }, { call, put, select }) {
      const { data } = yield call(
        documentService.addDocumentAttachments,
        values
      )
      message.success(data.message, 3)
      yield put({ type: 'reload' })
    ...
    *reload (action, { put, select }) {
      ...
      const filterStr = yield select(
        state => state.workAttendanceList.filterStr
      )
