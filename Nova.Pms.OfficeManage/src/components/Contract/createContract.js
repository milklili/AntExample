...
function createContract({ dispatch, cityData, streetRegisterData, streetWarkingData, contacts, contract, files, loading}) {
    var options = {
                        ...
                        <Row gutter={8}>
                            <Col span={24}>
                                <FormItem {...formItemRow} label="注册地址">
                                    {registerProvinceOptions.length > 0 && getFieldDecorator('registeredProvince', {
                                        initialValue: contract.registeredProvince ? contract.registeredProvince : "所在省",
                                    ...
                                    {registerCityOptions.length > 0 && getFieldDecorator('registeredCity', {
                                        initialValue: contract.registeredCity ? contract.registeredCity : "所在市",
                                        rules: [{ required: true, message: 'Please input the registered address!' }]
                                    })(<Select style={{ width: 120 }} onChange={this.onRegisterCityChange}>{registerCityOptions}</Select>)}
                                    {registerAreaOptions.length > 0 && getFieldDecorator('registeredTown', {
                                        initialValue: contract.registeredTown ? contract.registeredTown : "所在县",
                                        rules: [{ required: true, message: 'Please input the registered address!' }]
                                    })(<Select style={{ width: 120 }} onChange={this.onRegisterAreaChange}>{registerAreaOptions}</Select>)}
                                    {registerTownOptions.length > 0 && getFieldDecorator('registeredStreet', {
                                        initialValue: contract.registeredStreet ? contract.registeredStreet : "所在街道",
                                        rules: [{ required: true, message: 'Please input the registered address!' }]
                        ...
                        <Row gutter={8}>
                            <Col span={24}>
                                <FormItem {...formItemRow} label="办公地址">
                                    {workingProvinceOptions.length > 0 && getFieldDecorator('workingProvince', { initialValue: contract.workingProvince ? contract.workingProvince : "所在省" })
                                        (<Select style={{ width: 120 }} onChange={this.handleWorkingProvinceChange}>{workingProvinceOptions}</Select>)}
                                    {workingCityOptions.length > 0 && getFieldDecorator('workingCity', { initialValue: contract.workingCity ? contract.workingCity : "所在市" })
                                        (<Select style={{ width: 120 }} onChange={this.onWorkingCityChange}>{workingCityOptions}</Select>)}
                                    {workingAreaOptions.length > 0 && getFieldDecorator('workingTown', { initialValue: contract.workingTown ? contract.workingTown : "所在县" })
                                        (<Select style={{ width: 120 }} onChange={this.onWorkingAreaChange}>{workingAreaOptions}</Select>)}
                                    {workingTownOptions.length > 0 && getFieldDecorator('workingStreet', { initialValue: contract.workingStreet ? contract.workingStreet : "所在街道" })
                            ...
                            <Col span={12}>
                                <FormItem {...formItemLayout} label="合同生效日期">
                                    {getFieldDecorator('contractBeginTime', {
                                        initialValue: moment(contract.contractBeginTime ? contract.contractBeginTime : new Date(), 'YYYY-MM-DD'),
                            ...
                            <Col span={12}>
                                <FormItem {...formItemLayout} label="合同结束日期">
                                    {getFieldDecorator('contractEndTime', {
                                        initialValue: moment(contract.contractEndTime ? contract.contractEndTime : new Date(), 'YYYY-MM-DD')
                                    })(<DatePicker />)}
                                </FormItem>
                            </Col>
                        </Row>
