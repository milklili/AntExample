...
const CollectionEditForm = Form.create()(props => {
          ...
          <Col span={12}>
            <FormItem {...formItemLayout} label="性别">
              {getFieldDecorator('sex', {
                initialValue: userData.sex == 0 || userData.sex == '男'
                  ? '男'
                  : '女',
              })(
                <Select style={{ width: 120 }}>
...
function createContract ({
  ...
  var registerOptions = Object.assign({}, options),
    workOptions = Object.assign({}, options)

  var provinceObj = areaList.province.find(
    province => province.name == contract.registeredProvince
  )
  if (provinceObj) {
  ...
  var cityObj = registerAreaList.city.find(
    city => city.name == contract.registeredCity
  )
  if (cityObj) {
    registerOptions.city = cityObj.code
    registerAreaList = GetAreaList(registerOptions, cityData)
  }
  var areaObj = registerAreaList.area.find(
    city => city.name == contract.registeredTown
  )
  if (areaObj) {
  ...
  for (let street in streetRegisterData) {
    registerAreaList.town.push({
      code: street,
      name: streetRegisterData[street],
    })
  }

  var provinceObj = areaList.province.find(
    province => province.name == contract.workingProvince
  )
  if (provinceObj) {
  ...
  var cityObj = workAreaList.city.find(
    city => city.name == contract.workingCity
  )
  if (cityObj) {
    workOptions.city = cityObj.code
    workAreaList = GetAreaList(workOptions, cityData)
  }
  var areaObj = workAreaList.area.find(
    city => city.name == contract.workingTown
  )
  if (areaObj) {
  ...
  const props = {
    ...
    onChange (info) {
      if (info.file.status === 'done') {
        ...
        dispatch({
          type: 'createContract/addAttachment',
          payload: { file: info.file.response, model: contract },
        })
      } else if (info.file.status === 'removed') {
        dispatch({
          type: 'createContract/removeAttachment',
  ...
  class NormalContractFrom extends React.Component {
    ...
    handleRegisterProvinceChange = value => {
      ...
      contract.registeredProvince = registerAreaList.province.find(
        province => province.code == value
      ).name
      contract.registeredCity = registerAreaList.city.length > 0
        ? registerAreaList.city[0].name
        : '所在市'
      contract.registeredTown = registerAreaList.area.length > 0
        ? registerAreaList.area[0].name
        : '所在县'
      contract.registeredStreet = registerAreaList.town.length > 0
        ? egisterAreaList.town[0].name
        : '所在街道'
    };
    onRegisterCityChange = value => {
      registerOptions.city = value
      registerAreaList = GetAreaList(registerOptions, cityData)
      contract.registeredCity = registerAreaList.city.find(
    ...
    onRegisterAreaChange = value => {
      contract.registeredTown = registerAreaList.area.find(
        area => area.code == value
      ).name
      contract.registeredStreet = '所在街道'
      dispatch({
        type: 'createContract/GetStreet',
    ...
    handleWorkingProvinceChange = value => {
      ...
      contract.workingProvince = workAreaList.province.find(
        province => province.code == value
      ).name
      contract.workingCity = workAreaList.city.length > 0
        ? workAreaList.city[0].name
        : '所在市'
      contract.workingTown = workAreaList.area.length > 0
        ? workAreaList.area[0].name
        : '所在县'
      contract.workingStreet = workAreaList.town.length > 0
        ? workAreaList.town[0].name
        : '所在街道'
    };
    onWorkingCityChange = value => {
      ...
      contract.workingCity = workAreaList.city.find(
        city => city.code == value
      ).name
      contract.workingTown = workAreaList.area.length > 0
        ? workAreaList.area[0].name
    ...
    onWorkingAreaChange = value => {
      contract.workingTown = workAreaList.area.find(
        area => area.code == value
      ).name
    ...
    render () {
      ...
      const registerProvinceOptions = registerAreaList.province.map(
        province => <Option key={province.code}>{province.name}</Option>
      )
      const registerCityOptions = registerAreaList.city.map(city => (
        <Option key={city.code}>{city.name}</Option>
      ))
      const registerAreaOptions = registerAreaList.area.map(area => (
        <Option key={area.code}>{area.name}</Option>
      ))
      const registerTownOptions = registerAreaList.town.map(town => (
        <Option key={town.code}>{town.name}</Option>
      ))

      const workingProvinceOptions = workAreaList.province.map(province => (
        <Option key={province.code}>{province.name}</Option>
      ))
      const workingCityOptions = workAreaList.city.map(city => (
        <Option key={city.code}>{city.name}</Option>
      ))
      const workingAreaOptions = workAreaList.area.map(area => (
        <Option key={area.code}>{area.name}</Option>
      ))
      const workingTownOptions = workAreaList.town.map(town => (
        <Option key={town.code}>{town.name}</Option>
      ))
      return (
        <div>
          <Form onSubmit={this.handleSubmit}>
                ...
                <FormItem {...formItemLayout} label="法人代表">
                  {getFieldDecorator('legalRepresentative', {
                    ...
                    rules: [
                      {
                        required: true,
                        message: 'Please input the legal Representative!',
                      },
                    ],
                  })(<Input />)}
                </FormItem>
            ...
            <Row gutter={8}>
              <Col span={24}>
                <FormItem {...formItemRow} label="注册地址">
                  {registerProvinceOptions.length > 0 &&
                    getFieldDecorator('registeredProvince', {
                      initialValue: contract.registeredProvince
                        ? contract.registeredProvince
                        : '所在省',
                      rules: [
                        {
                          required: true,
                          message: 'Please input the registered address!',
                        },
                      ],
                    })(
                      <Select
                        style={{ width: 120 }}
                        onChange={this.handleRegisterProvinceChange}
                      >
                        {registerProvinceOptions}
                      </Select>
                    )}
                  {registerCityOptions.length > 0 &&
                    getFieldDecorator('registeredCity', {
                      initialValue: contract.registeredCity
                        ? contract.registeredCity
                        : '所在市',
                      rules: [
                        {
                          required: true,
                      ...
                      <Select
                        style={{ width: 120 }}
                        onChange={this.onRegisterCityChange}
                      >
                        {registerCityOptions}
                      </Select>
                    )}
                  {registerAreaOptions.length > 0 &&
                    getFieldDecorator('registeredTown', {
                      initialValue: contract.registeredTown
                        ? contract.registeredTown
                        : '所在县',
                      rules: [
                        {
                          required: true,
                      ...
                      <Select
                        style={{ width: 120 }}
                        onChange={this.onRegisterAreaChange}
                      >
                        {registerAreaOptions}
                      </Select>
                    )}
                  {registerTownOptions.length > 0 &&
                    getFieldDecorator('registeredStreet', {
                      initialValue: contract.registeredStreet
                        ? contract.registeredStreet
                        : '所在街道',
                      rules: [
                        {
                          required: true,
            ...
            <Row gutter={8}>
              <Col span={24}>
                <FormItem {...formItemRow} label="办公地址">
                  {workingProvinceOptions.length > 0 &&
                    getFieldDecorator('workingProvince', {
                      initialValue: contract.workingProvince
                        ? contract.workingProvince
                        : '所在省',
                    })(
                      <Select
                        style={{ width: 120 }}
                        onChange={this.handleWorkingProvinceChange}
                      >
                        {workingProvinceOptions}
                      </Select>
                    )}
                  {workingCityOptions.length > 0 &&
                    getFieldDecorator('workingCity', {
                      initialValue: contract.workingCity
                        ? contract.workingCity
                        : '所在市',
                    })(
                      <Select
                        style={{ width: 120 }}
                        onChange={this.onWorkingCityChange}
                      >
                        {workingCityOptions}
                      </Select>
                    )}
                  {workingAreaOptions.length > 0 &&
                    getFieldDecorator('workingTown', {
                      initialValue: contract.workingTown
                        ? contract.workingTown
                        : '所在县',
                    })(
                      <Select
                        style={{ width: 120 }}
                        onChange={this.onWorkingAreaChange}
                      >
                        {workingAreaOptions}
                      </Select>
                    )}
                  {workingTownOptions.length > 0 &&
                    getFieldDecorator('workingStreet', {
                      initialValue: contract.workingStreet
                        ? contract.workingStreet
                        : '所在街道',
                    })(
                      <Select style={{ width: 120 }}>
                        {workingTownOptions}
              ...
              <Col span={12}>
                <FormItem {...formItemLayout} label="数据库名">
                  {getFieldDecorator('database', {
                    ...
                    rules: [
                      { required: true, message: 'Please input the database!' },
                    ],
                  })(<Input />)}
                </FormItem>
              </Col>
                ...
                <FormItem {...formItemLayout} label="合同生效日期">
                  {getFieldDecorator('contractBeginTime', {
                    ...
                    rules: [
                      {
                        required: true,
                        message: 'Please input the contract begin time!',
                      },
                    ],
                  })(<DatePicker />)}
                </FormItem>
            ...
            <Row gutter={8}>
              <Col span={12}>
                <Row gutter={8}>
                  <Col span={8}>
                    <span className={styles.editRow}>合同附件:</span>
                  </Col>
                  <Col span={16}>
                    <Upload {...props}>
            ...
            <Row gutter={8}>
              ...
              <Col span={20}>
                <Row gutter={8}>
                  ...
                  <Col span={12}>
                    <FormItem {...formItemLayout} label="性别">
                      {getFieldDecorator('primarySex', {
                        initialValue: contract.primarySex == 0 ||
                          contract.primarySex == '男'
                          ? '男'
                          : '女',
                      })(
                        <Select style={{ width: 120 }}>
      ...
      switch (Object.keys(changedFields)[0]) {
        ...
        case 'legalRepresentative':
          contract.legalRepresentative =
            changedFields.legalRepresentative.value
          break

        case 'uniformSocialCreditCode':
          contract.uniformSocialCreditCode =
            changedFields.uniformSocialCreditCode.value
          break
...
function mapStateToProps (state) {
  ...
  var files = []
  contract.attachments.forEach(currentFile => {
    var file = {}
...
function GetAreaList (options, cityData) {
  ...
  for (var code in cityData) {
    if (!(code % 1e4)) {
      // 获取所有的省级行政单位
      province.push({ code, name: cityData[code] })
      ...
      if (options.province && p > 0 && p < 1e4) {
        ...
        if (!(code % 100)) {
          hasCity = true
          city.push({ code, name: cityData[code] })
          ...
          if (options.city && c > 0 && c < 100) {
            // 同个城市的地区
            area.push({ code, name: cityData[code] })
          ...
          city.push({ code, name: cityData[code] }) // 直辖市
          if (options.area) {
