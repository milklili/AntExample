...
function InitiatedList ({
  ...
  class EditInitiated extends React.Component {
    ...
    uploadAttachments = {
      ...
      defaultFileList: approval.attachments
        .map(attachment => {
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              ...
              <Popconfirm
                title="确定要撤销该审批记录?"
                onConfirm={this.revokedApproval.bind(null, [record.id])}
              >
                <a>撤销</a>
              </Popconfirm>
              &nbsp;
              <Popconfirm
                title="确定要删除该审批记录?"
                onConfirm={this.deleteApproval.bind(null, [record.id])}
              >
                <a>删除</a>
              </Popconfirm>
